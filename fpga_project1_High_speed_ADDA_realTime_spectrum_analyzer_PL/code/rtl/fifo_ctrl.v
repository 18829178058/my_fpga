...
parameter   CNT_RD = 511;
parameter   CNT_WR = 511;
...
//wire define    
wire [8:0]  rd_data_count;  //读端FIFO剩余数据量
...
assign s_axis_data_tlast = ( (cnt_rd_en[8:0] == CNT_RD) && ad_data_out_en ) ? 1'b1 : 1'b0; 
...
always @(posedge axi_clk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0)
        rd_en <= 1'b0;
    else if(cnt_rd_en >= CNT_RD)
        rd_en <= 1'b0;     
    else if(rd_data_count >=  CNT_RD - 1'b1) 
        rd_en <= 1'b1;
    else
        rd_en <= rd_en; 
end  
