module e2prom_rw(
	...
	//user interface
	output reg rw_done , //EEPROM 读写测试完成
	output reg rw_result //EEPROM 读写测试结果 0:失败 1:成功
	);
	
	//parameter define
	//EEPROM 写数据需要添加间隔时间,读数据则不需要
	...
	//EEPROM 读写测试,先写后读，并比较读出的值与写入的值是否一致
	...
	if(wait_cnt == (WR_WAIT_TIME - 14'b1)) begin //EEPROM 写操作延时完成
	wait_cnt <= 14'b0;
