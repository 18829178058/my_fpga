//iic通用模块,100K速度
/*************************端口定义*******************************/
module iic(clk, rst_n, start, worr, sendnum, recvnum, data_in, data_out, done_send, done_recv, done_iic, sda, scl);
...
/*************************参数声明*******************************/	
	...
	parameter	IDW		=	8'b1010101_0;	//ID拼写方向
	parameter	IDR		=	8'b1010101_1;	//ID拼读方向
			...
			case(c_state)
				...
				ACK1	:	begin
								if(ack_flag	== 1)	//应答无效
									...
									if(cnt == delay - 1 && worr == 0 && ack_flag == 0)
										n_state	=	SEND;
									else
										if(cnt == delay - 1 && worr == 1 && ack_flag == 0)
											n_state	=	RECV;
										else
											n_state	=	c_state;
							end
				...
				ACK2	:	begin
								if(ack_flag == 1)	//应答无效
									...
									if(ack_flag == 0 && cnt == delay - 1 && send_cnt == sendnum && recvnum == 0)//写完数据后无需读
										n_state = STOP;
									else
										if(ack_flag == 0 && cnt == delay - 1 && send_cnt == sendnum && recvnum != 0)//写完数据后需要读
											n_state	=	START1;
										else
...
//状态机第三段，时序逻辑描述输出
		...
		begin
			if(!rst_n)
				...
				case(c_state)
					...
					RECV	:	begin	
									if(cnt == MID)
										data_out[7-cnt_bit] <= sda_in;//读的数据
	...
	assign	done_send	=	(c_state == ACK2 && cnt == MID)	?	1:0;
	assign	done_recv	=	(c_state	==	ACK3 && cnt	==	MID)	?	1:0;
	assign	done_iic		=	(c_state == STOP && cnt == MID)	?	1:0;
