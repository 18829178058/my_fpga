module move #(parameter	BIN_WIDTH	=	6,
	...
	wire	[BCD_WIDTH + BIN_WIDTH - 1	:	0]	temp;
	...
	generate	
		for(i = 0; i < BCD_WIDTH/4; i = i + 1)
			begin	:	a
				adjust a1(.adjust_in(move_in[BIN_WIDTH+BCD_WIDTH-1-4*i:BIN_WIDTH+BCD_WIDTH-4-4*i]),
							 .adjust_out(temp[BIN_WIDTH+BCD_WIDTH-1-4*i:BIN_WIDTH+BCD_WIDTH-4-4*i]));
			end
	endgenerate
	
	assign	temp[BIN_WIDTH - 1 : 0]	=	move_in[BIN_WIDTH - 1 : 0];  //BIN部分不变
	
	assign	move_out	=	temp << 1;								 //左移一位
