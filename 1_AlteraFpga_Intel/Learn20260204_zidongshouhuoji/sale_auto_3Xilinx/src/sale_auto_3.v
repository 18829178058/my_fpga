...
(
				...
				//传感器输入信号
				input			s0,	//0.5元硬币投币 ，默认通过按键模拟
				input			s1,	//1元硬币投币，默认通过按键模拟
				//出货找零
...
always 			@(*)begin
				state_ns			=	ML_INPUT;
				case(state_cs)
					ML_INPUT		:if(s0_tmp|s1_tmp)
											state_ns =   ML_INPUT_OFF;
									 else 
											state_ns =   ML_INPUT ;
					
					ML_0P5			:if(s0_tmp|s1_tmp)
											state_ns =   ML_INPUT_OFF;
									 else 
											state_ns =   ML_0P5;
					ML_1			:if(s0_tmp|s1_tmp)
											state_ns =   ML_INPUT_OFF;
									 else 
											state_ns =   ML_1;
					ML_INPUT_OFF	:if({s1_tmp,s0_tmp}==2'b00)
										case(total_tmp)
...
always 			@(posedge		clk	or			negedge			rst_n)
				...
				else   if((state_ns==ML_INPUT_OFF)&({s1,s0}==2'b00))
						case(total)
...
always 			@(posedge		clk	or			negedge			rst_n)
				if(rst_n==1'b0)
						...
						cnt1 <=  30'd0;
				else  if(  (cnt1==0) &  led_z0p5_tmp ) 
						cnt1 <=cnt1 +  1'b1;
				else 
						cnt1 <=cnt1 +  1'b1;
assign 			led_sale= cnt0>0  ?  1'b1:1'b0; 
assign 			led_z0p5= cnt1>0  ?  1'b1:1'b0; 
endmodule 